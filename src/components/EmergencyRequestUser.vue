...
export default {
  ...
  methods: {
    ...
    addMarker(event) {
      ...
      const popupContent = this.createPopupContent(markerCoordinates, gpsCoordinates, this.userInfo);
    ...
    transformToGps(x, y) {
      ...
      if (isNaN(lat) || isNaN(lon)) {
        console.error(`GPS coordinates are NaN for SVG coordinates: (${x}, ${y})`);
      }
    ...
    bilinearInterpolation(x1, x2, x3, x4, y1, y2, y3, y4, x, y) {
      ...
      const result = a0 + a1 * (x - x1) + a2 * (y - x1) + a3 * (x - x1) * (y - x1);
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          position => {
            const latitude = position.coords.latitude;
            const longitude = position.coords.longitude;
            this.addMarkerFromGps(latitude, longitude, this.userInfo);
            document.getElementById('coordinatesDisplay').innerHTML = `Latitude: ${latitude}, Longitude: ${longitude}`;
          },
          error => {
            console.error("Error getting location", error.message);
